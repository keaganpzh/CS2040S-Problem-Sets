...
public class GameTree {
    ...
    int findValue() {
        // TODO: Implement this
        return findValueHelper(this.root, Player.ONE);
    }

    int findValueHelper(TreeNode node, Player player) {
        if (node == null) {
            ...
            if (player == Player.ONE) {
                // BEST CASE: Player ONE chooses the child node with the highest value.
                int max = Integer.MIN_VALUE;
                for (int i = 0; i < node.numChildren; i++) {
                    int childValue = findValueHelper(node.children[i], other(player));
                    if (childValue > max) {
                        max = childValue;
                    }
                }
                ...
                // BEST CASE: Player TWO chooses the child node with the lowest value.
